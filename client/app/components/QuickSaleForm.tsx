...
import { X, Plus, Minus, ShoppingCart, Check, UserPlus } from "lucide-react";
...
export default function QuickSaleForm({ onClose }: QuickSaleFormProps) {
	...
	const handleCreateCustomerQuick = async () => {
		if (!customerName) {
			handleError("Enter a name to create customer", { title: "Validation Error" });
			return;
		}
		try {
			const created = await customersApi.create({ name: customerName });
			setSelectedCustomerId(created.id);
			setCustomers([created, ...customers]);
			handleSuccess("Customer created");
		} catch (err: any) {
			handleError(err, { title: "Failed to create customer" });
		}
	};
	...
	const handleSubmit = async () => {
		...
		// Enforce customer for full debt sales
		if (paymentStatus === "debt" && !selectedCustomerId) {
			handleError("Select or create a customer for debt sales", {
				title: "Customer required for debt"
			});
			return;
		}
		...
		<div className="fixed inset-0 bg-black/50 flex items-start justify-center z-[9999]">
			<div className="bg-white rounded-t-[2rem] w-full h-[calc(100vh-16px)] mt-4 flex flex-col animate-in fade-in-0 duration-200">
				...
				<div className="p-6 overflow-y-auto space-y-6">
					...
					<div className="space-y-2">
						<Label className="text-base font-medium">Customer (optional)</Label>
						<div className="flex gap-2 items-center">
							<Select value={selectedCustomerId ? String(selectedCustomerId) : undefined} onValueChange={(val) => setSelectedCustomerId(val ? Number(val) : null)}>
								<SelectTrigger className="h-12 text-base rounded-xl min-w-[200px]">
							...
							<Button variant="outline" onClick={handleCreateCustomerQuick} title="Create customer from name">
								<UserPlus className="w-4 h-4 mr-2" /> Create
							</Button>
						</div>
						{paymentStatus === 'debt' && !selectedCustomerId && (
							<p className="text-sm text-red-600">Customer is required for debt sales.</p>
						)}
					</div>
